...
export default class RestaurantModel extends BaseModel {
  ...
  constructor(data = {}) {
    ...
    this.#status = this._string(
      data.status,
      data.is_open === 0 ? "closed" : "open",
    );
  }
  get owner_id() {
  ...
  set owner_id(v) {
    this.#owner_id = v ?? null;
  }
  get name() {
    return this.#name;
  }
  set name(v) {
    this.#name = this._string(v);
  }
  get category() {
    return this.#category;
  }
  set category(v) {
    this.#category = this._string(v);
  }
  get description() {
    return this.#description;
  }
  set description(v) {
    this.#description = this._string(v);
  }
  get image_url() {
    return this.#image_url;
  }
  set image_url(v) {
    this.#image_url = this._string(v);
  }
  get status() {
    return this.#status;
  }
  set status(v) {
    this.#status = this._string(v, "open");
  }
  isOpen() {
    return this.#status.toLowerCase() === "open" || this.#status === "1";
  }
  getDisplayName() {
