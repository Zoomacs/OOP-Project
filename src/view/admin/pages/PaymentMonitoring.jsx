...
export default function PaymentMonitoring() {
  ...
  const filtered = useMemo(
    () =>
      payments.filter((p) =>
        `${p.id} ${p.user} ${p.restaurant} ${p.method} ${p.status}`
          .toLowerCase()
  ...
  const revenue = payments.reduce(
    (sum, p) =>
      sum + Number(p.amount_value || String(p.amount).replace(" EGP", "")),
    0,
  );
  ...
  function edit(payment) {
    setForm({
      id: payment.raw_id || String(payment.id).replace("#TRX-", ""),
  ...
  return (
    <>
      <PageHeader
        title="Payments CRUD"
        subtitle="Create, read, update, and delete payment records."
      >
        <input
          className="search"
          value={search}
            ...
            {filtered.map((p) => (
              <tr key={p.id}>
                ...
                <td className="row-actions">
                  ...
                  <button className="btn small dark" onClick={() => remove(p)}>
                    Delete
                  </button>
                </td>
              </tr>
