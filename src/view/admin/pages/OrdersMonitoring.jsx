...
export default function OrdersMonitoring() {
  ...
  const filtered = useMemo(
    () =>
      orders.filter((o) =>
        `${o.id} ${o.restaurant} ${o.customer} ${o.status}`
          .toLowerCase()
  ...
  return (
    <>
      <PageHeader
        title="Orders CRUD"
        subtitle="Read all orders, change status, edit order values, and delete incorrect records."
      >
        <input
          className="search"
          value={search}
      ...
      <div className="grid">
        ...
        <StatCard
          title="Completed"
          value={
            orders.filter((o) => ["received", "delivered"].includes(o.status))
              .length
          }
          badge="Done"
          badgeClass="blue"
            ...
            {filtered.map((order) => (
              <tr key={order.id}>
                  ...
                  <Badge
                    type={
                      order.status === "cancelled"
                        ? "red"
                        : ["received", "delivered"].includes(order.status)
                          ? "green"
                          : order.status === "ready"
                            ? "blue"
                            : "gold"
                ...
                <td className="row-actions">
                  <button className="btn small" onClick={() => edit(order)}>
                    Edit
                  </button>
                  <button
                    className="btn small green"
                    onClick={() => quickStatus(order, "ready")}
                  >
                    Ready
                  </button>
                  <button
