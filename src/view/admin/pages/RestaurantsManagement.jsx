  ...
  const filtered = useMemo(
    () =>
      restaurants.filter((r) =>
        `${r.name} ${r.owner_name} ${r.owner_email} ${r.category}`
          .toLowerCase()
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Create, read, update, delete, open/close, and assign restaurants to owners."
      >
        <input
          className="search"
          value={search}
        ...
        <form className="form admin-form" onSubmit={submit}>
          ...
          <label className="check-card">
            <input
              type="checkbox"
              name="staff_delivery"
              checked={Number(form.staff_delivery) === 1}
              onChange={change}
            />{" "}
            Staff delivery
          </label>
          <div className="crud-actions full">
            ...
            {editing && (
              <button
                type="button"
                className="btn ghost"
                onClick={() => setForm(emptyRestaurant)}
              >
                Cancel Edit
              </button>
            )}
            ...
            {filtered.map((r) => (
              <tr key={r.id}>
                ...
                <td className="row-actions">
                  ...
                  <button className="btn small dark" onClick={() => remove(r)}>
                    Delete
                  </button>
                </td>
              </tr>
