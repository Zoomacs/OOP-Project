    ...
    <div className="dashboard-page">
      ...
      <div className="dashboard-two-col">
        <Card className="table-wrap">
          <h3>Latest Orders</h3>
          <table>
            <thead><tr><th>Order</th><th>Restaurant</th><th>Customer</th><th>Status</th></tr></thead>
            <tbody>{orders.map((order) => <tr key={order.id}><td>#{order.id}</td><td>{order.restaurant}</td><td>{order.customer}</td><td><Badge type={order.status === "cancelled" ? "red" : order.status === "ready" ? "blue" : "gold"}>{order.status}</Badge></td></tr>)}</tbody>
          </table>
        </Card>
        <Card>
          <h3>Support Tickets</h3>
          {tickets.map((ticket) => <div className="ticket" key={ticket.id}><div><b>{ticket.title}</b><p className="muted">{ticket.email}</p></div><Badge type={ticket.status === "Urgent" ? "red" : ticket.status === "Pending" ? "gold" : "green"}>{ticket.status}</Badge></div>)}
        </Card>
      </div>
    </div>
  );
