...
import { ChefHat, Utensils, CheckCircle2, Clock } from "lucide-react";
import { api } from "../../api";
...
function OrderTrack({ page }) {
  ...
  const isInstaPay = baseOrderData?.payment_method === "instapay";
  const isConfirming = isInstaPay && liveStatus === "pending";
  ...
  if (!baseOrderData) return (
    <div className="ot-page-container">
      <button className="details-btn back-nav" onClick={() => navigate("/orderhistory")}>&larr; Back to Orders</button>
      <h2>No order selected</h2>
    </div>
  );
  const isPreparing = liveStatus === "preparing" || liveStatus === "pending";
      ...
      <div className="track-wrapper">
        ...
        <div className="track-hero">
          <div className="head-text">
            <h1>{isConfirming ? <>Awaiting payment<br />confirmation</> : <>Your feast is<br />being prepared!</>}</h1>
          </div>
          <div className="estimated-card">
            <p>ORDER TIME</p>
            <h2>{String(baseOrderData.time || new Date().toLocaleTimeString()).split(",").pop()}</h2>
          </div>
        </div>
        <div className="status-container">
          {isConfirming ? (
            <div className="status-step active">
              <div className="step-icon"><Clock size={28} /></div>
              <h3 className="step-title">Confirming Order</h3>
              <p className="step-desc">Waiting for staff to confirm payment</p>
            </div>
          ) : (
            <>
              <div className={`status-step ${isPreparing || isReady || isDelivered ? "completed" : "active"}`}>
                <div className="step-icon"><ChefHat size={28} /></div>
              ...
              <div className={`status-step ${isReady ? "active" : isDelivered ? "completed" : ""}`}>
                <div className="step-icon"><Utensils size={28} /></div>
              ...
              <div className={`status-step ${isDelivered ? "completed" : ""}`}>
                <div className="step-icon"><CheckCircle2 size={28} /></div>
          ...
          <div className="summary-items">
            <div className="summary-item-row">
              <div className="item-info">
                <h4>Order #{baseOrderData.id}</h4>
                <p>Status: {isConfirming ? "confirming payment" : liveStatus}</p>
                <p className="unit-price">Total: {baseOrderData.total || baseOrderData.total_amount || 0} EGP</p>
              </div>
            </div>
          </div>
        </div>
