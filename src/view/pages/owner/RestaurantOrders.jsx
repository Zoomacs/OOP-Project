...
function RestaurantOrders() {
  ...
  const loadOrders = useCallback(() => {
    return api(`restaurant-orders&restaurant_id=${restaurantId}`)
  ...
  async function updateStatus(targetId, status) {
    await api("update-status", {
      method: "POST",
      body: JSON.stringify({ order_id: targetId, status }),
    });
    ...
    if (status === "received" || status === "cancelled") {
      ...
      setOrders((prev) =>
        prev.map((order) =>
          order.id === targetId ? { ...order, state: status } : order,
        ),
      );
    }

    if (
      selectedOrder?.id === targetId &&
  ...
  const renderOrderCard = (order) => (
    <div className="orders-list" key={order.id}>
      ...
      <div className="card-actions">
        {order.state === "pending" && (
          <>
            <button className="view-btn" onClick={() => handleOpenDetails(order)}>
              Details
            </button>
  ...
  return (
    <div className="active-orders-container">
      ...
      <div className="columns-wrapper">
        {["pending", "preparing", "ready"].map((status) => (
          <div className="order-column" key={status}>
            <h2 className={`list-name ${status}-title`}>
              {status[0].toUpperCase() + status.slice(1)}{" "}
              <span>({orders.filter((o) => o.state === status).length})</span>
            </h2>

            <div className="list">
              {orders.filter((o) => o.state === status).map(renderOrderCard)}

              {orders.filter((o) => o.state === status).length === 0 && (
                <p className="empty-state">No {status} orders</p>
              )}
            </div>
          </div>
        ))}
        ...
        {selectedOrder ? (
          <div className="drawer-content">
            <div className="drawer-customer-info">
              <img src={selectedOrder.senderImg || userImg} alt="" />
            ...
            <div className="drawer-section">
              ...
              <p>
                <strong>Method:</strong> {selectedOrder.paymentType}
              </p>
      ...
      {isDetailsOpen && (
        <div className="drawer-overlay" onClick={handleCloseDetails}></div>
      )}
    </div>
  );
}
