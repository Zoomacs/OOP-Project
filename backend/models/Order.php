...
class Order extends DomainModel
{
    ...
    {
        $this->total = $this->numberValue($value);
        return $this;
    }
    ...
    {
        $this->payment_method = $this->stringValue($value, 'cash');
        return $this;
    }
    ...
    {
        $this->payment_status = $this->stringValue($value, 'pending');
        return $this;
    }
    ...
    {
        $this->items = is_array($value) ? $value : [];
        return $this;
    }
    public function isActive()
    {
        return !in_array(strtolower($this->status), ['completed', 'cancelled', 'delivered', 'received']);
    }
    public function isValid()
    {
        return $this->user_id && $this->restaurant_id && $this->total >= 0;
    }
    public function toArray()
    {
        return ['id' => $this->id, 'user_id' => $this->user_id, 'restaurant_id' => $this->restaurant_id, 'total' => $this->total, 'status' => $this->status, 'payment_method' => $this->payment_method, 'payment_status' => $this->payment_status, 'items' => $this->items];
    }
}
